...
ARTICLE_HTML_PATH = os.path.join(PROJECT_DIR, "articles", "article_gender_trust_v3_极简.html")
TITLE = "她被未婚夫迷奸3次，还叫别的男人一起"
AUTHOR = "AI提效实验室"
DIGEST = "德国华人迷奸案宣判：医学博士邵之霆被判5年。三次迷奸未婚妻、带其他男性参与、全程录像分享——985精英把专业知识变成了害人配方。这不是恐怖片，是昨天刚宣判的真实案件。"
COVER_IMAGE_PATH = os.path.join(PROJECT_DIR, "covers", "A_dark__atmospheric_editorial__2026-07-09T08-20-52.png")
...
def replace_image_urls(html_content, mapping):
    """用微信 CDN URL 替换 HTML 中的本地图片 src"""
    if not mapping:
        return html_content
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # 重新打开编辑器，避免之前的临时上传内容干扰
        print("\n[4] 重新打开编辑器，准备粘贴正文...")
        token = await extract_token(page)
        new_url = f"https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit&action=edit&type=77&isNew=1&token={token}&lang=zh_CN"
        await page.goto(new_url, wait_until="domcontentloaded", timeout=15000)
        await page.wait_for_timeout(5000)
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "editor_new_replaced.png"))

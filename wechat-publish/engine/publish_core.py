#!/usr/bin/env python3
"""微信公众号 Playwright 发布核心（题材无关，由 runs/*.json 驱动）。"""
import asyncio
import json
import os
import re
...
from .config import PROJECT_DIR, PublishConfig
...
_current: PublishConfig | None = None


def _cfg() -> PublishConfig:
    if _current is None:
        raise RuntimeError("PublishConfig 未初始化")
    return _current


def save_draft_url(url, appmsg_id=None, title=None):
    data = {"url": url, "appmsg_id": appmsg_id, "title": title or _cfg().title, "slug": _cfg().slug, "topic": _cfg().topic}
...
async def publish_from_editor(page):
    ...
    for round_idx in range(MAX_ROUNDS):
        ...
        await page.screenshot(path=str(SCREENSHOT_DIR / f"publish_dialog_{round_idx}.png"))
...
async def run(config: PublishConfig):
    global _current
    _current = config
    cfg = config
    ...
    with open(cfg.html_path, "r", encoding="utf-8") as f:
        html_content = f.read()

    images_dir = str(cfg.images_dir_path) if cfg.images_dir_path else None
    ...
    async with async_playwright() as p:
        ...
        await page.screenshot(path=str(SCREENSHOT_DIR / "editor_new.png"))

        print("\n[3] 上传正文图片到微信 CDN...")
        if images_dir:
            image_mapping = await upload_inline_images(page, images_dir, html_content)
        else:
            image_mapping = {}
        if image_mapping:
            ...
            print("  ⚠️ 没有图片被上传（请检查 images_dir 与 HTML 中的文件名）")
        ...
        await page.goto(new_url, wait_until="domcontentloaded", timeout=15000)
        await page.wait_for_timeout(5000)
        await page.screenshot(path=str(SCREENSHOT_DIR / "editor_new_replaced.png"))

        print("\n[5] 填写标题...")
        await fill_title(page, cfg.title)

        print("\n[6] 填写作者...")
        await fill_author(page, cfg.author)
        ...
        print("\n[8] 上传封面...")
        cover_url, file_id = await upload_cover_via_toolbar(page, str(cfg.cover_path))
        if not cover_url:
            print("❌ 封面上传失败")
            await context.close()
            return
        await set_cover(page, cover_url, file_id)
        await page.screenshot(path=str(SCREENSHOT_DIR / "cover_set.png"))

        print("\n[9] 填写摘要...")
        await fill_digest(page, cfg.digest)

        print("\n[10] 保存草稿...")
        appmsg_id = await save_draft(page)
        await page.screenshot(path=str(SCREENSHOT_DIR / "after_save.png"), full_page=True)
        ...
        await page.screenshot(path=str(SCREENSHOT_DIR / "cover_restored.png"))
        ...
        await page.screenshot(path=str(SCREENSHOT_DIR / "publish_final.png"), full_page=True)

        try:
            body_text = await page.inner_text("body")
            if cfg.title in body_text:
                print("    ✅ 页面中找到文章标题")

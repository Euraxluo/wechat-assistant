...
微信公众号自动发布 - 最终版本
关键点：
1. 创建新文章（旧草稿 100002578 已损坏）
2. 封面上传：通过编辑器工具栏上传图片，获取真实 file_id 和 CDN URL
3. 手动设置封面：隐藏占位按钮，设置背景图
4. 保存后重新加载，再恢复背景图（不保存）
5. 从编辑器点击 mass_send 发表
"""
import asyncio, json, os, re
from playwright.async_api import async_playwright

ARTICLE_HTML_PATH = "/Users/echo/project/wechat-skills/article_排版_摸鱼绿(moyu-green).html"
TITLE = "同事用AI 2小时干完你1天的工作"
AUTHOR = "AI提效实验室"
DIGEST = "不是AI替代你，是你不用AI被替代。7个真实场景，教你把6小时的活压缩到10分钟。"
...
async def publish_from_editor(page):
    ...
    for round_idx in range(10):
        ...
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, f"final_dialog_{round_idx}.png"))
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "final_editor_new.png"))
        ...
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "final_cover_set.png"))

        print("\n[7] 保存草稿...")
        appmsg_id = await save_draft(page)
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "final_after_save.png"), full_page=True)
        ...
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "final_cover_restored.png"))
        ...
        await page.screenshot(path=os.path.join(SCREENSHOT_DIR, "final_result.png"), full_page=True)
